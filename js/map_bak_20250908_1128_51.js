...
let map;
let marker;
// Single overlay reference (Polygon or Rectangle)
window.zipOverlay = window.zipOverlay || null;
    ...
    try {
        ...
        if (mapContainer && !document.getElementById('zip-boundary-toggle')) {
            ...
            cb.addEventListener('change', function(){
                ...
                if (!vis && window.zipOverlay) { try { window.zipOverlay.setMap(null); } catch(_){} }
                if (vis && window.zipOverlay) { try { window.zipOverlay.setMap(map); } catch(_){} }
            });
        }
        ...
        geocoderInstance.geocode(req, function(results, status) {
            ...
            if (status === 'OK' && results && results[0]) {
                ...
                try {
                    ...
                    // Replace any existing overlay with a Rectangle for strong visibility
                    ...
                    window.zipOverlay = new google.maps.Rectangle({
                        ...
                        zIndex: 1000,
                        map: (window.showZipBoundary !== false) ? map : null
                    });
                    try {
                        map.fitBounds(rbounds);
                        const rz = map.getZoom();
                        if (rz) map.setZoom(Math.min(15, Math.max(10, rz)));
                    } catch(_){ }
