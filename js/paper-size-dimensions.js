/* START OF CODE - Cline - 2025-05-23 15:53:03 File: js/paper-size-dimensions.js */
 ...
 * 10. FIXED: Prevents duplicate Paper Auto-Size row from being created
 */
 ...
 * FIXED: Check if Paper Auto-Size row already exists before creating it
 */
function initPaperSizeAndDPI() {
    console.log("Initializing Paper Auto-Size and DPI functionality");
    
    // FIXED: Check if Paper Auto-Size row already exists and remove it if it does
    const existingPaperAutoSize = document.getElementById('paper-auto-size');
    if (existingPaperAutoSize) {
        console.log("Paper Auto-Size row already exists, removing and recreating");
        const parentRow = existingPaperAutoSize.closest('.settings-row');
        if (parentRow) {
            parentRow.remove();
        }
    }
    ...
    paperSizeRow.className = 'paper-size-row settings-row';
    paperSizeRow.innerHTML = `
        ...
        <select id="dpi-selector">
            ...
            <option value="200">200 (Print)</option>
            <option value="300" selected>300 (Hi-Res)</option>
        </select>
    `;
...
/* END OF CODE - Cline - 2025-05-23 15:53:03 File: js/paper-size-dimensions.js */
