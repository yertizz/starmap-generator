...
(function () {
  /* START OF CODE - Codex - 2025-09-07 17:12 File: js/zip_boundary_patch.js */
  ...
  function bindZipHandlersOrFallback() {
    var original = window.geocodeZip;
    // If a robust geocodeZip already exists (e.g., from map.js), do not override it; just ensure inputs call it
    ...
    window.geocodeZip = function (zipText) {
      ...
      window.geocoderInstance.geocode({ address: zipText }, function (results, status) {
        if (status === 'OK' && results && results[0]) {
          ...
          // best-effort polygon fetch with extended timeout (12s) to accommodate Overpass latency
          (function(){
            try {
              var c = 'US'; var cc = document.getElementById('country-code'); if (cc && cc.value) c = String(cc.value).trim();
              // Build absolute path based on current page path
              var basePath = (function(){ try { var p = window.location.pathname; return p.slice(0, p.lastIndexOf('/')+1); } catch(_) { return '/star-map/'; } })();
              var url = basePath + 'proxy/zip_polygon.php?postalcode='+encodeURIComponent(zipText)+'&country='+encodeURIComponent(c);
              try {
                // Append lat/lng if available for rectangle server fallback
                if (typeof window.map==='object' && window.map) {
                  var ctr = window.map.getCenter && window.map.getCenter();
                  if (ctr) { url += '&lat=' + encodeURIComponent(ctr.lat()) + '&lng=' + encodeURIComponent(ctr.lng()); }
                }
              } catch(_){ }
              if (window.DEBUG_POLY) console.log('DEBUG_POLY: fetching polygon', url);
              var t = new Promise(function(_,rej){ setTimeout(function(){ rej(new Error('timeout')); }, 12000); });
                ...
                .then(function(gj){
                  if (gj && gj.type==='Feature' && gj.geometry && Array.isArray(gj.geometry.coordinates)){
                    ...
                    if (gj.geometry.type==='Polygon') {
                      pathsArr = gj.geometry.coordinates.map(function(ring){ return ring.map(function(c){ return new google.maps.LatLng(c[1], c[0]); }); });
                    } else if (gj.geometry.type==='MultiPolygon') {
                      if (gj.geometry.coordinates.length>0) {
                        pathsArr = gj.geometry.coordinates[0].map(function(ring){ return ring.map(function(c){ return new google.maps.LatLng(c[1], c[0]); }); });
                      }
                    }
                    if (pathsArr.length){
                      window.zipPolygon = new google.maps.Polygon({ paths: pathsArr, strokeColor:'#FF0000', strokeOpacity:0.8, strokeWeight:2, fillColor:'#FF0000', fillOpacity:0.25, map: (window.showZipBoundary!==false && window.map) ? window.map : null });
                      try {
                        var bnd = new google.maps.LatLngBounds();
                        pathsArr.forEach(function(r){ r.forEach(function(pt){ bnd.extend(pt); }); });
                        if (window.map){ window.map.fitBounds(bnd); var z=window.map.getZoom(); if(z) window.map.setZoom(Math.min(15, Math.max(10, z))); }
                      } catch(_){ }
                    }
                  }
                })
  ...
  function start() { injectUI(); bindZipHandlersOrFallback(); }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', start); else start();
  /* END OF CODE - Codex - 2025-09-07 17:12 File: js/zip_boundary_patch.js */
})();
