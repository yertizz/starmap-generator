...
let map;
let marker;
// ensure global polygon reference for toggles
window.zipPolygon = window.zipPolygon || null;
let mapInitialized = false;
let ignoreNextCenterChange = false;
let geocoderInstance;
let showZipBoundary = true; // default ON for polygon overlay
    ...
    try {
        ...
        if (mapContainer && !document.getElementById('zip-boundary-toggle')) {
            ...
            cb.addEventListener('change', function(){
                showZipBoundary = !!this.checked;
                if (!showZipBoundary && window.zipPolygon) { try { window.zipPolygon.setMap(null); } catch(_){} }
                if (showZipBoundary && window.zipPolygon) { try { window.zipPolygon.setMap(map); } catch(_){} }
            });
        }
...
async function geocodeZip(zipText) {
    try {
        if (!geocoderInstance) geocoderInstance = new google.maps.Geocoder();
        geocoderInstance.geocode({ address: zipText }, function(results, status) {
            if (status === 'OK' && results && results[0]) {
                const res = results[0];
                ...
                // Draw immediate rectangle fallback from bounds/viewport
                try {
                    const b = res.geometry && (res.geometry.bounds || res.geometry.viewport);
                    if (b) {
                        const ne = b.getNorthEast();
                        const sw = b.getSouthWest();
                        const nw = new google.maps.LatLng(ne.lat(), sw.lng());
                        const se = new google.maps.LatLng(sw.lat(), ne.lng());
                        const path = [ne, se, sw, nw, ne]; // closed loop
                        ...
                        window.zipPolygon = new google.maps.Polygon({
                            ...
                            fillColor: '#FF0000',
                            fillOpacity: 0.25,
                            map: showZipBoundary ? map : null
                        });
                    }
                } catch(e){ console.warn('Rectangle fallback draw failed:', e); }
                ...
                (async function(){
                    try {
                        ...
                        const url = `proxy/zip_polygon.php?postalcode=${encodeURIComponent(zipText)}&country=${encodeURIComponent(country)}`;
                        const resp = await fetch(url, { headers: { 'Accept': 'application/json' } });
                if (resp.ok) {
                    const gj = await resp.json();
                    if (gj && gj.type === 'Feature' && gj.geometry && Array.isArray(gj.geometry.coordinates)) {
                                if (window.zipPolygon) { try { window.zipPolygon.setMap(null); } catch(_){} }
                                // Support Polygon (outer ring at coordinates[0])
                                const rings = gj.geometry.type === 'Polygon' ? [gj.geometry.coordinates] : gj.geometry.coordinates;
                                const paths = rings[0][0].map(function(c){ return new google.maps.LatLng(c[1], c[0]); });
                                window.zipPolygon = new google.maps.Polygon({
                                    paths: paths,
                                    strokeColor: '#FF0000',
                                    strokeOpacity: 0.8,
