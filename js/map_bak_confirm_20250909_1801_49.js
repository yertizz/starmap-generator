...
function initializeGoogleMapInternal() { // Renamed internally
    ...
    try { setPolygonStatus('Ready'); setTimeout(function(){ setPolygonStatus('Ready'); }, 500); } catch(_){}
}
...
async function geocodeZip(zipText) {
    try {
        ...
        try { window.__LAST_ACTION = { type:'zip', ts: thisTs }; } catch(_){}
        ...
        try { window.__CURRENT_ZIP = String(zipText); } catch(_){}
        try { window.__ZIP_INFLIGHT = reqId; window.__LAST_ZIP = String(zipText); } catch(_){}
        if (!geocoderInstance) geocoderInstance = new google.maps.Geocoder();
        setPolygonStatus('Fetching location and boundary…');
        // Use the original, reliable ZIP geocode without country restriction
        const req = { address: zipText };
        geocoderInstance.geocode(req, function(results, status) {
            try { if (window.__ZIP_INFLIGHT !== reqId) return; } catch(_){ }
            try { if (!window.__LAST_ACTION || window.__LAST_ACTION.ts !== thisTs || window.__LAST_ACTION.type !== 'zip') return; } catch(_){ }
            if (status === 'OK' && results && results[0]) {
                ...
                updateCoordinates(lat, lng);
                // Update address and place fields from this matched result
                try {
                    const addrEl = document.getElementById('address'); if (addrEl && res.formatted_address) addrEl.value = res.formatted_address;
                    var city='', st='', ctry='';
                    try {
                        (res.address_components||[]).forEach(function(c){
                            if ((c.types||[]).indexOf('locality')>=0 || (c.types||[]).indexOf('postal_town')>=0) city = c.long_name;
                            else if ((c.types||[]).indexOf('administrative_area_level_1')>=0) st = c.short_name || c.long_name;
                            else if ((c.types||[]).indexOf('country')>=0) ctry = c.long_name || c.short_name;
                        });
                    } catch(_){}
                    try { window.__CURRENT_CITY = city; window.__CURRENT_STATE = st; window.__CURRENT_COUNTRY = ctry; } catch(_){}
                } catch(_){ }
                ...
                try {
                    ...
                    if (viewport) {
                        ...
                        try { window.__LAST_CORNERS = [ne, new google.maps.LatLng(sw.lat(), ne.lng()), sw, new google.maps.LatLng(ne.lat(), sw.lng())]; } catch(_){}
                        try { map.fitBounds(viewport); const rz = map.getZoom(); if (rz) map.setZoom(Math.min(15, Math.max(10, rz))); } catch(_){ }
                        setPolygonStatusDiag('Zip/Postal Code Boundary Drawn');
                        try {
                            const nw = new google.maps.LatLng(ne.lat(), sw.lng());
                            const se = new google.maps.LatLng(sw.lat(), ne.lng());
                            addTempCornerMarkers([ne,se,sw,nw], 5000);
                        } catch(_){ }

                        // Style Google DDS Boundaries POSTAL_CODE layer (only when enabled)
                        try {
                            if (USE_DDS && map && typeof map.getFeatureLayer === 'function') {
                                const placeId = res.place_id;
                                const postalLayer = map.getFeatureLayer('POSTAL_CODE');
                                if (postalLayer && typeof postalLayer.style !== 'undefined') {
                                    const zipNorm = String(zipText).replace(/\s+/g,'').toUpperCase();
                                    const featureStyle = { strokeColor: '#FF0000', strokeOpacity: 1.0, strokeWeight: 3, fillColor: '#FF0000', fillOpacity: 0.25 };
                                    postalLayer.style = ({ feature }) => {
                                        try {
                                            const pidMatch = !!placeId && (feature.placeId === placeId);
                                            let nameMatch = false;
                                            try {
                                                const dn = (feature.displayName || '').toString().replace(/\s+/g,'').toUpperCase();
                                                nameMatch = (dn === zipNorm);
                                            } catch(_){}
                                            return (pidMatch || nameMatch) ? featureStyle : null;
                                        } catch(_) { return null; }
                                    };
                                } else {
                                    setPolygonStatus('DDS POSTAL_CODE not configured for this Map ID; showing viewport rectangle only');
                                }
                            }
                        } catch(_){ }
                ...
                try {
                    ...
                    try { if (window.zipOverlay) window.zipOverlay.setMap(null); } catch(_){}
                    const rbounds = new google.maps.LatLngBounds(sw, ne);
                    ...
                    try { window.__LAST_CORNERS = [ne,se,sw,nw]; } catch(_){ }
                    try { map.fitBounds(rbounds); const rz = map.getZoom(); if (rz) map.setZoom(Math.min(15, Math.max(10, rz))); } catch(_){ }
                    setPolygonStatusDiag('Zip Code Polygon Boundary Unavailable For this Zip Code: Displaying Fallback Rectangle.');
                    return;
                } catch(_){ }
                    ...
                    try {
                        ...
                        window.zipOverlay = new google.maps.Polygon({
                            ...
                            map: map
                                    ...
                                    try {
                                        if (gj.properties && gj.properties.source === 'fallback-rectangle') setPolygonStatusDiag('Zip Code Polygon Boundary Unavailable For this Zip Code: Displaying Fallback Rectangle.');
                                        else if (gj.properties && gj.properties.source) setPolygonStatusDiag('Polygon source: ' + gj.properties.source);
                                        else setPolygonStatusDiag('Zip/Postal Code Boundary Drawn');
                                    } catch(_){}
                                    return;
                                }
                            }
...
function geocodeAddress(address) {
    ...
    const addrTs = Date.now();
    try { window.__LAST_ACTION = { type:'addr', ts: addrTs }; } catch(_){}
    // If a ZIP was just drawn, delay address geocode slightly to avoid races
    try {
        if (window.__LAST_ZIP_TS && (Date.now() - window.__LAST_ZIP_TS) < 1200) {
            const a = address; setTimeout(function(){ try { geocodeAddress(a); } catch(_){} }, 1200); return;
        }
        if (!(window.google && google.maps)) { setPolygonStatus('Initializing map…'); window.__PENDING_GEO.push({ type:'addr', value: address }); return; }
    } catch(_){}
    const geocoder = new google.maps.Geocoder();
    
    geocoder.geocode({ address: address }, function(results, status) {
        try { if (!window.__LAST_ACTION || window.__LAST_ACTION.ts !== addrTs || window.__LAST_ACTION.type !== 'addr') return; } catch(_){}
        if (status === "OK" && results && results.length > 0) {
            ...
            // Clear prior ZIP overlays to avoid stale boundary after address recenter
            try { if (window.zipOverlay) window.zipOverlay.setMap(null); } catch(_){}
            try { if (window.zipRect) window.zipRect.setMap(null); } catch(_){}
            ...
            if (zipInput) {
                ...
                try { window.__CURRENT_ZIP = ''; window.__CURRENT_CITY = city; window.__CURRENT_STATE = state; window.__CURRENT_COUNTRY = country; } catch(_){}
                try { setPolygonStatus(`Ready — Address centered: ${[city,state].filter(Boolean).join(' ')}${country?'. '+country:''}`); } catch(_){}
            });
        } else {
