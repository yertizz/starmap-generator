...
async function saveCurrentSettings() { // MODIFIED: Added async
    ...
    try {
        ...
        const userIdForSettings = (typeof getCurrentUserId === 'function') ? getCurrentUserId() : 'default_user';
        const settings = {
            userId: userIdForSettings,
            ...
            // Paper size and DPI
            paperAutoSize: getVal("paper-auto-size", ''),
            dpi: (function(){
                const dpiSel = document.getElementById('dpi-selector') || document.getElementById('dpi');
                return dpiSel ? (dpiSel.value || '') : '';
            })(),
            ...
            if (USE_SERVER) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        console.warn(`Server save failed: ${response.status} ${response.statusText}. ${errorText}`);
                    }
                } catch (e) {
                    console.warn('Server save threw, will use local fallback:', e);
                }
            }
            if (!saved) {
                try {
                    const userId = (typeof getCurrentUserId === 'function') ? getCurrentUserId() : 'default_user';
                    localStorage.setItem(`${userId}_app_settings`, settingsJSON);
                    saved = true;
                    console.log('Settings saved locally under key', `${userId}_app_settings`);
                } catch (le) {
                    console.error('Local settings save failed:', le);
                }
            }
            if (saved) {
                alert('Settings Saved!');
            } else {
                alert('Unable to save settings. See console for details.');
            }
        } catch (fetchError) {
            console.error("Error saving settings:", fetchError);
...
async function loadSavedSettings() { // MODIFIED: Added async
    ...
    try {
        ...
        if (USE_SERVER) {
            try {
                const userIdForLoad = (typeof getCurrentUserId === 'function') ? getCurrentUserId() : 'default_user';
                const response = await fetch(`settings/settings_manager.php?userId=${encodeURIComponent(userIdForLoad)}`, {
                    method: 'GET',
                ...
                console.warn('Server load failed, will try local settings:', e);
            }
        }
        if (!settings) {
            try {
                const userId = (typeof getCurrentUserId === 'function') ? getCurrentUserId() : 'default_user';
                const raw = localStorage.getItem(`${userId}_app_settings`);
                if (raw) settings = JSON.parse(raw);
            } catch (le) {
                console.warn('Local settings load failed:', le);
            }
        }
        if (!settings || typeof settings !== 'object' || Object.keys(settings).length === 0) {
            console.log('No saved settings found in server or local storage.');
            return false;
        }
    } catch (fetchError) {
        console.error("Error retrieving settings:", fetchError);
...
// NOTE: Auto-save removed to give users full control over when to save.
