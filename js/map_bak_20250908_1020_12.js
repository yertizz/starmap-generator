    ...
    try {
        ...
        if (mapContainer && !document.getElementById('zip-boundary-toggle')) {
            ...
            cb.addEventListener('change', function(){
                ...
                if (!vis && window.zipPolygon) { try { window.zipPolygon.setMap(null); } catch(_){} }
                if (vis && window.zipPolygon) { try { window.zipPolygon.setMap(map); } catch(_){} }
            });
        }
        ...
        geocoderInstance.geocode(req, function(results, status) {
            ...
            if (status === 'OK' && results && results[0]) {
                ...
                try {
                    ...
                    // Do not clear until we draw the new overlay
