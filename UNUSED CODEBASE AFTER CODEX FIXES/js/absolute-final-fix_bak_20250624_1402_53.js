...
(function() {
    ...
    // 8. FIX COMBINED VIEW OPTIONS SPACING TO MATCH MOCKUP EXACTLY
    function fixCombinedViewSpacingToMatchMockup() {
        console.log('🎯 Fixing Combined View Options spacing to match mockup exactly...');
        ...
        // Apply EXACT spacing to match mockup
        const container = combinedRow.querySelector('div[style*="background-color"]');
        if (container) {
            // CRITICAL: Reduce padding to match mockup's tight spacing
            container.style.cssText = `
                width: 100% !important;
            ...
            // Fix title spacing to match mockup
            const title = container.querySelector('div:first-child');
            if (title) {
                title.style.cssText = `
                    font-weight: bold !important;
            ...
            // CRITICAL: Fix Circle Overlap spacing to match mockup
            let sliderRow = null;
            const allDivs = container.querySelectorAll('div');
            for (const div of allDivs) {
                if (div.querySelector('#circle-overlap-percent')) {
            ...
            // CRITICAL: Fix Map Order spacing to match mockup - MUCH CLOSER
            let mapOrderRow = null;
            const allDivs2 = container.querySelectorAll('div');
            for (const div of allDivs2) {
                if (div.querySelector('input[name="map-order"]')) {
            ...
            if (mapOrderRow) {
                // EXACT spacing to match mockup - very close to Circle Overlap
                mapOrderRow.style.cssText = `
                    display: block !important;
                    width: 100% !important;
        ...
        // Fix form border cutoff to match mockup
        ...
        if (container2) {
            container2.style.maxWidth = '900px';
            container2.style.width = '900px';
            container2.style.overflow = 'visible';
        }
        
        console.log('✅ Combined View Options spacing fixed to match mockup exactly');
    }
