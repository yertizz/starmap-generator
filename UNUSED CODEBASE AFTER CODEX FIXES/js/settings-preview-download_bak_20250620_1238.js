...
function viewStarStreetLandscape() {
    console.log("🌄 SIMPLE Combined Landscape view called");
    ...
    // Draw simple placeholder for now - NO COMPLEX RENDERING
    const bgColor = document.getElementById('bg-color-canvas').value || '#F5F5DC';
    ctx.fillStyle = bgColor;
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ...
    ctx.fillText("Combined Landscape View", canvas.width / 2, canvas.height / 2);
    ctx.fillText("(Simplified - No Flickering)", canvas.width / 2, canvas.height / 2 + 30);
    
    // Enable the download button
    document.getElementById('download-star-street-landscape-btn').disabled = false;
    
    console.log("Simple landscape view rendered");
}
