...
// (Keeping this function as it was in the restored backup for now)
function generateStarMap() {
    ...
    setTimeout(() => {
        try {
            ...
            const latFormatted = getVal("latitude", "");
            const lonFormatted = getVal("longitude", "");
            ...
            if (isNaN(latDecimal) || isNaN(lonDecimal)) {
                 alert(`Invalid coordinate format in input fields.\nPlease ensure Latitude and Longitude use the format like NDD° MM′.MMMMM or WDDD° MM′.MMMMM\nReceived: Lat='${latFormatted}', Lon='${lonFormatted}'`);
                 throw new Error(`Invalid coordinate format in input fields: Lat='${latFormatted}', Lon='${lonFormatted}'`);
            }
            ...
            if (latFormatted && lonFormatted && typeof drawText === 'function') {
                 const formattedCoords = `${latFormatted} ${lonFormatted}`;
...
function initMap() {
    ...
    try {
        ...
        // const defaultLat = 38.1015;
        // const defaultLng = -94.6934;
        const defaultLat = 37.30000;
        const defaultLng = -95.3000;
        const defaultZoom = 4;
        ...
        // Function to update UI elements with coordinates (from restored version)
        const updateCoordinateUI = (lat, lng) => {
            const latInput = document.getElementById('latitude');
            const lngInput = document.getElementById('longitude');
            const display = document.getElementById('latLongDisplay');

            let formattedCoords = `Lat: ${lat.toFixed(6)} | Long: ${lng.toFixed(6)}`;
            let latFormattedPart = `Lat: ${lat.toFixed(6)}`;
            let lonFormattedPart = `Long: ${lng.toFixed(6)}`;

            if (typeof formatCoordinates === 'function') {
                 formattedCoords = formatCoordinates(lat, lng);
                 const parts = formattedCoords.split(' ');
                 if (parts.length >= 2) {
                     latFormattedPart = parts[0];
                     lonFormattedPart = parts[1];
                 } else {
                     console.warn("Could not split formatted coordinates correctly for input fields.");
                     latFormattedPart = formattedCoords;
                     lonFormattedPart = '';
                 }
            } else {
                 console.warn("formatCoordinates function not found in utils.js when updating UI.");
                 latFormattedPart = lat.toFixed(6);
                 lonFormattedPart = lng.toFixed(6);
            }

            if (latInput) latInput.value = latFormattedPart;
            if (lngInput) lngInput.value = lonFormattedPart;
