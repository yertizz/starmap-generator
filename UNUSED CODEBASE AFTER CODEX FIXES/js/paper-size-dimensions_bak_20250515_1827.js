/* START OF CODE - Cline - 2025-05-15 18:16:00 File: js/paper-size-dimensions.js */
 ...
 * 8. FIXED: Uses MutationObserver to prevent other scripts from changing the defaults
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // AGGRESSIVE FIX: Delay execution to ensure this runs after any other scripts
    setTimeout(function() {
        ...
        // AGGRESSIVE FIX: Add MutationObserver to monitor for changes to the inputs
        addMutationObserver();
...
/**
 * AGGRESSIVE FIX: Add MutationObserver to monitor for changes to the inputs
 * This will prevent other scripts from changing the default dimensions and aspect ratio
 */
function addMutationObserver() {
    console.log("Adding MutationObserver to monitor for changes to the inputs");
    ...
    // Create a MutationObserver to monitor for changes to the value attribute
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            if (mutation.type === 'attributes' && mutation.attributeName === 'value') {
                const target = mutation.target;
                
                // Check if the width input was changed
                if (target === widthInput && parseInt(target.value) !== expectedValues.width) {
                    console.log(`Width input was changed to ${target.value}, resetting to ${expectedValues.width}`);
                    target.value = expectedValues.width;
                }
                
                // Check if the height input was changed
                if (target === heightInput && parseInt(target.value) !== expectedValues.height) {
                    console.log(`Height input was changed to ${target.value}, resetting to ${expectedValues.height}`);
                    target.value = expectedValues.height;
                }
            }
        });
    });
    
    // Create a MutationObserver to monitor for changes to the checked attribute
    const checkboxObserver = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            if (mutation.type === 'attributes' && mutation.attributeName === 'checked') {
                const target = mutation.target;
                
                // Check if the aspect ratio checkbox was changed
                if (target === aspectRatioCheckbox && target.checked !== expectedValues.aspectRatio) {
                    console.log(`Aspect ratio checkbox was changed to ${target.checked}, resetting to ${expectedValues.aspectRatio}`);
                    target.checked = expectedValues.aspectRatio;
                }
            }
        });
    });
    ...
    observer.observe(widthInput, { attributes: true });
    observer.observe(heightInput, { attributes: true });
    checkboxObserver.observe(aspectRatioCheckbox, { attributes: true });
    
    // Also monitor for changes to the value property
    widthInput.addEventListener('input', function() {
        if (parseInt(this.value) === 3000) {
            console.log(`Width input was changed to 3000, resetting to ${expectedValues.width}`);
            this.value = expectedValues.width;
        }
    });
    
    heightInput.addEventListener('input', function() {
        if (parseInt(this.value) === 3000) {
            console.log(`Height input was changed to 3000, resetting to ${expectedValues.height}`);
            this.value = expectedValues.height;
        }
    });
    
    // Also monitor for changes to the checked property
    aspectRatioCheckbox.addEventListener('change', function() {
        if (this.checked !== expectedValues.aspectRatio) {
            console.log(`Aspect ratio checkbox was changed to ${this.checked}, resetting to ${expectedValues.aspectRatio}`);
            this.checked = expectedValues.aspectRatio;
        }
    });
    
    // AGGRESSIVE FIX: Periodically check the values and reset if necessary
    setInterval(function() {
        if (parseInt(widthInput.value) !== expectedValues.width) {
            console.log(`Width input was changed to ${widthInput.value}, resetting to ${expectedValues.width}`);
            widthInput.value = expectedValues.width;
        }
        
        if (parseInt(heightInput.value) !== expectedValues.height) {
            console.log(`Height input was changed to ${heightInput.value}, resetting to ${expectedValues.height}`);
            heightInput.value = expectedValues.height;
        }
        
        if (aspectRatioCheckbox.checked !== expectedValues.aspectRatio) {
            console.log(`Aspect ratio checkbox was changed to ${aspectRatioCheckbox.checked}, resetting to ${expectedValues.aspectRatio}`);
            aspectRatioCheckbox.checked = expectedValues.aspectRatio;
        }
...
function setDefaultDimensions() {
    ...
    // AGGRESSIVE FIX: Set default dimensions to 2550 x 3300
    ...
    // AGGRESSIVE FIX: Ensure aspect ratio checkbox is unchecked by default
    ...
    // AGGRESSIVE FIX: Explicitly trigger a change event to ensure any listeners are notified
    ...
    // AGGRESSIVE FIX: Directly modify any inline styles that might be affecting the aspect ratio
...
/* END OF CODE - Cline - 2025-05-15 18:16:00 File: js/paper-size-dimensions.js */
