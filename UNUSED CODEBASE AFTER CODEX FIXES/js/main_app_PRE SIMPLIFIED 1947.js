// START OF CODE - Cline - 2025-04-19 19:09 File: js/main_app.js
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add Zoom Slider Functionality (Temporarily Disabled)
    ...
    if (zoomSlider && zoomValueDisplay && starMapCanvas) {
        zoomSlider.addEventListener('input', function() {
            ...
            // starMapCanvas.style.transform = `scale(${scaleValue})`; // Temporarily disabled
        });
        // Set initial zoom on load
        const initialZoomLevel = zoomSlider.value;
        zoomValueDisplay.textContent = initialZoomLevel;
        // starMapCanvas.style.transformOrigin = 'center center';
        // starMapCanvas.style.transform = `scale(${initialZoomLevel / 100})`; // Temporarily disabled
        console.log("App Zoom feature temporarily disabled in JS.");
    } else {
...
// --- Core Star Map Generation (Using createImageBitmap & Canvas Transform) ---
async function generateStarMap() {
    console.log("generateStarMap called - using LST, createImageBitmap, and canvas transform");
    ...
    // Clear canvas and reset generated state *before* getting dimensions
    ...
    try {
        ...
        // Resize PREVIEW canvas ATTRIBUTES if necessary
        ...
        // --- Apply Transform to counteract canvas aspect ratio distortion ---
        ctx.save(); // Save state before transform
        const aspect = canvas.width / canvas.height; // Calculate aspect ratio
        let transformedCenterX = centerX; // Initialize with original centerX
        let transformedCenterY = centerY; // Initialize with original centerY

        // Apply scaling based on aspect ratio
        if (aspect > 1) { // Wider than tall
            ctx.scale(1 / aspect, 1);
            transformedCenterX = centerX * aspect; // Adjust X coordinate in scaled space
            console.log(`Applied horizontal scale correction: ${1 / aspect}`);
        } else if (aspect < 1) { // Taller than wide
            ctx.scale(1, aspect);
            transformedCenterY = centerY / aspect; // Adjust Y coordinate in scaled space
            console.log(`Applied vertical scale correction: ${aspect}`);
        }
        // If aspect is 1, no scaling needed, transformed coordinates remain same as original

        // --- Draw Border (in transformed space) ---
        if (borderWidth > 0) {
            console.log(`Drawing border: Width=${borderWidth}, Color=${borderColor}`);
            ctx.strokeStyle = borderColor;
            // Scale linewidth slightly to compensate for transform, aiming for visual consistency
            ctx.lineWidth = borderWidth * Math.min(aspect, 1/aspect) || borderWidth; // Use calculated aspect
            ctx.beginPath();
            ctx.arc(transformedCenterX, transformedCenterY, radius, 0, Math.PI * 2, false); // Use transformed coords
            ctx.stroke();
        }

        // --- Apply Clip Path (in transformed space) ---
        ctx.beginPath();
        ctx.arc(transformedCenterX, transformedCenterY, radius, 0, Math.PI * 2, false); // Use transformed coords
        ctx.closePath();
        ctx.clip();

        // --- Image Scaling Logic (Fit inside circle, maintain aspect ratio) ---
        const circleDiameter = radius * 2;
        // Use the "fit inside" scaling logic
        const imgScaleFactor = Math.min(circleDiameter / imageBitmap.width, circleDiameter / imageBitmap.height); // Ensure entire image fits
        const drawWidth = imageBitmap.width * imgScaleFactor;
        const drawHeight = imageBitmap.height * imgScaleFactor;
        // Center the image within the transformed circle center
        const drawX = transformedCenterX - drawWidth / 2; // Use transformed center
        const drawY = transformedCenterY - drawHeight / 2; // Use transformed center

        // Draw the image (in transformed space)
        ctx.drawImage(imageBitmap, drawX, drawY, drawWidth, drawHeight);
        console.log(`DEBUG: Drawing imageBitmap scaled to fit clip diameter. Original: ${imageBitmap.width}x${imageBitmap.height}, Scale: ${imgScaleFactor.toFixed(4)}, Drawn: ${drawWidth.toFixed(2)}x${drawHeight.toFixed(2)} at ${drawX.toFixed(2)},${drawY.toFixed(2)}`);
        imageBitmap.close(); // Close bitmap to free memory

        // --- Restore state to remove transform and clip ---
        ctx.restore(); // Restore state (removes scale and clip)
        ...
        // Calculate text start position based on original centerY and radius
        const circleBottomY = centerY + radius + (borderWidth / 2);
...
// --- END OF CODE - Cline - 2025-04-19 18:12 File: js/main_app.js