...
function setupEventDropdownSuggestions(selectId, storageKey, suggestionsContainerId, maxItems = 50) {
    ...
    // Protected events (not deletable) - built-in events
    ...
    // Show suggestions when select is focused/clicked
    selectElement.addEventListener('focus', async function() {
        console.log(`DEBUG HISTORY: Event dropdown #${selectId} focused, showing suggestions`);
    ...
    if (userHistory.length > 0) {
        ...
        userHistory.forEach(item => {
            ...
            const isProtected = protectedEvents.some(protected => 
                item.toLowerCase().includes(protected.replace(/_/g, ' ')) ||
                protected.replace(/_/g, ' ').includes(item.toLowerCase())
            );
