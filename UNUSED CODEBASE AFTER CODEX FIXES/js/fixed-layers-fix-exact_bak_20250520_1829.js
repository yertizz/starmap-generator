/* START OF CODE - Cline - 2025-05-20 18:22:52 File: js/fixed-layers-fix-exact.js */
...
function fixFixedLayersStyling() {
    try {
        ...
        // Find the Fixed Layers heading
        const fixedLayersHeading = document.querySelector('h3.fixed-layers-heading');
        if (fixedLayersHeading) {
            fixedLayersHeading.style.textAlign = 'center';
            fixedLayersHeading.style.color = '#0275d8';
            fixedLayersHeading.style.margin = '15px 0';
            fixedLayersHeading.style.fontSize = '18px';
        }
        ...
        inputGroups.forEach((group, index) => {
            ...
            displayValue.style.width = '220px'; // Reduced width to prevent wrapping
            displayValue.style.minWidth = '220px';
            displayValue.style.maxWidth = '220px';
            displayValue.style.height = '30px';
            displayValue.style.lineHeight = '30px';
            displayValue.style.padding = '0 10px';
            displayValue.style.marginRight = '10px';
            displayValue.style.backgroundColor = '#f0f0f0';
            displayValue.style.border = '1px solid #ddd';
            displayValue.style.borderRadius = '4px';
            displayValue.style.fontSize = '14px';
            displayValue.style.color = '#333';
            displayValue.style.fontFamily = 'inherit';
            displayValue.style.whiteSpace = 'nowrap';
            displayValue.style.overflow = 'hidden';
            displayValue.style.textOverflow = 'ellipsis';
...
function fixDateDisplay() {
    try {
        ...
        if (textPlacementsDateDisplay) {
            ...
            console.log("Updated Text Placements date display to:", correctDate);
        } else {
            console.error("Text Placements date display element not found");
        }
        
        // Also try to find and update any other date display elements
        const allDateDisplays = document.querySelectorAll('.date-display, .date-value, .date-text');
        if (allDateDisplays.length > 0) {
            allDateDisplays.forEach(element => {
                element.textContent = correctDate;
            });
            console.log(`Updated ${allDateDisplays.length} additional date display elements`);
        }
        
        // Try to find the date in the Text Placements section by its row
        const textPlacementsRows = document.querySelectorAll('#text-placements-container tr');
...
/* END OF CODE - Cline - 2025-05-20 18:22:52 File: js/fixed-layers-fix-exact.js */
