...
function viewStarStreetLandscape() {
    ...
    const borderColor = document.getElementById('border-color').value || "#FDCA0D"; // Border color from settings
    const streetMapBorderColor = borderColor; // Use the same border color for street map
    const starMapBorderColor = borderColor; // Use the same border color for star map
    
    // Draw left circle (Street Map) border first
    ctx.beginPath();
    ctx.arc(leftCenterX, centerY, radius, 0, Math.PI * 2, true);
    ctx.closePath();
    ctx.lineWidth = borderWidth;
    ctx.strokeStyle = streetMapBorderColor;
    ctx.stroke();
    
    // Draw right circle (Star Map) border first
    ctx.beginPath();
    ctx.arc(rightCenterX, centerY, radius, 0, Math.PI * 2, true);
    ctx.closePath();
    ctx.lineWidth = borderWidth;
    ctx.strokeStyle = starMapBorderColor;
    ctx.stroke();
    
    // Create Google Maps Static API URL for the street map
    // Google Maps Static API has a maximum size of 640x640 pixels
    const mapSize = Math.min(radius * 2, 640); // Limit to 640x640 max
    const zoom = 12; // Default zoom level
    const mapType = "roadmap"; // Options: roadmap, satellite, terrain, hybrid
    ...
    // Create the Google Maps Static API URL
    const streetMapUrl = `https://maps.googleapis.com/maps/api/staticmap?center=${lat},${lng}&zoom=${zoom}&size=${mapSize}x${mapSize}&maptype=${mapType}&markers=color:red%7C${lat},${lng}&key=${apiKey}`;
    
    console.log("Loading Google Maps Static API image from:", streetMapUrl);
    ...
    streetMapImg.onload = function() {
        ...
        // Draw the street map image on our canvas, centered and scaled to fit the circle
        const aspectRatio = streetMapImg.width / streetMapImg.height;
        let drawWidth, drawHeight;
        ...
        console.log("Street map image loaded and drawn in landscape view");
    };
    
    // Set the street map image source to the Google Maps Static API URL
    streetMapImg.src = streetMapUrl;
    
    // Error handler for street map image
    ...
    // For the star map, we'll use the same approach as in the generateStarMap function
    // Calculate LST for Right Ascension
    let lstHours;
    if (typeof calculateLST === 'function') {
        lstHours = calculateLST(date, lng);
    ...
    // Prepare API Request Body for the star map
    const API_IMAGE_SIZE = 1000; // Smaller size for the combined view
    const requestBody = {
        style: starMapStyle,
        output: {
            width: API_IMAGE_SIZE,
    ...
    .then(blob => {
        ...
        starMapImg.onload = function() {
            ...
            // Draw the star map image on our canvas, centered and scaled to fit the circle
            const aspectRatio = starMapImg.width / starMapImg.height;
            let drawWidth, drawHeight;
            ...
            console.log("Star map image loaded and drawn in landscape view");
            
            // Revoke the object URL to free up memory
            URL.revokeObjectURL(starMapUrl);
        };
        
        starMapImg.onerror = function() {
            console.error("Failed to load star map image from blob");
            // Draw a fallback star map (navy blue circle)
            ctx.save();
            ctx.beginPath();
    ...
    console.log("Star Map + Street Map Landscape view rendering started");
}
...
function viewStarStreetPortrait() {
    ...
    // Get border settings
    const borderWidth = parseInt(document.getElementById('border-width').value) || 2;
    const streetMapBorderColor = document.getElementById('border-color').value || "#FDCA0D"; // Golden border for street map
    const starMapBorderColor = "#FFFFFF"; // White border for star map
    
    // Draw top circle (Street Map) border first
    ctx.beginPath();
    ...
    // Draw bottom circle (Star Map) border first
    ctx.beginPath();
    ...
    streetMapImg.onload = function() {
        ...
        console.log("Street map image loaded and drawn in portrait view");
    };
    ...
    .then(blob => {
        ...
        starMapImg.onload = function() {
            ...
            console.log("Star map image loaded and drawn in portrait view");
    ...
    console.log("Star Map + Street Map Portrait view rendering started");
}
