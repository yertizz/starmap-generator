...
(function() {
    ...
    function fixCombinedViewSpacingToMatchMockup() {
        ...
        // Apply ONLY spacing fixes to Combined View Options - NO OTHER CHANGES
        const container = combinedRow.querySelector('div[style*="background-color"]');
        if (container) {
            // ULTRA TIGHT spacing - even tighter than before
            container.style.cssText = `
                width: 100% !important;
            ...
            // ULTRA TIGHT Circle Overlap spacing
            let sliderRow = null;
            const allDivs = container.querySelectorAll('div');
            for (const div of allDivs) {
                if (div.querySelector('#circle-overlap-percent')) {
            ...
            // ULTRA TIGHT Map Order spacing - VERY CLOSE to Circle Overlap
            let mapOrderRow = null;
            const allDivs2 = container.querySelectorAll('div');
            for (const div of allDivs2) {
                if (div.querySelector('input[name="map-order"]')) {
