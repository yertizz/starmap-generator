...
(function() {
    ...
    function syncDateAndCoordinates() {
        ...
        // Initial updates with fallback values
        setTimeout(() => {
            updateDateDisplay();
            updateCoordinates();
            
            // Fallback: If date is still empty, set a default
            const fixedDate = document.getElementById('fixed-date-value');
            if (fixedDate && (!fixedDate.textContent || fixedDate.textContent.trim() === '')) {
                const dateInput = document.getElementById('date');
                if (dateInput && dateInput.value) {
                    ...
                    const months = ['January', 'February', 'March', 'April', 'May', 'June', 
                                   'July', 'August', 'September', 'October', 'November', 'December'];
                    const fallbackDate = `${months[month]} ${day}, ${year}`;
                    fixedDate.textContent = fallbackDate;
                    fixedDate.innerHTML = fallbackDate;
                    console.log('✓ Set fallback date:', fallbackDate);
                }
            }
        }, 100);
    ...
    // 8. FIX COMBINED VIEW OPTIONS SPACING ONLY - NO CONTAINER LAYOUT CHANGES
    function fixCombinedViewSpacingToMatchMockup() {
        console.log('🎯 Fixing ONLY Combined View Options spacing - NO container layout changes...');
        ...
        if (!combinedRow) {
            console.warn('⚠️ Combined View Options row not found');
            return;
        }
        
        // Apply ONLY spacing fixes to Combined View Options + FIX THICK BORDERS
        const container = combinedRow.querySelector('div[style*="background-color"]');
        if (container) {
            // ULTRA TIGHT spacing + SUBTLE BORDERS like other sections
            container.style.cssText = `
                width: 100% !important;
            ...
            // Fix title spacing - tighter + THICK BORDER
            const title = container.querySelector('div:first-child');
            if (title) {
                title.style.cssText = `
                    font-weight: bold !important;
            ...
            // ULTRA TIGHT Circle Overlap spacing + THICK BORDERS
            let sliderRow = null;
            const allDivs = container.querySelectorAll('div');
            for (const div of allDivs) {
                if (div.querySelector('#circle-overlap-percent')) {
            ...
            // ULTRA TIGHT Map Order spacing + SLIGHTLY THICKER BORDERS - VERY CLOSE to Circle Overlap
            let mapOrderRow = null;
            const allDivs2 = container.querySelectorAll('div');
            for (const div of allDivs2) {
                if (div.querySelector('input[name="map-order"]')) {
        ...
        console.log('✅ Combined View Options spacing ultra-tightened - NO container layout changes made');
    }

    // 9. FIX SETTINGS + PREVIEW + DOWNLOAD SECTION STYLING
    function fixSettingsPreviewDownloadStyling() {
        console.log('🎯 Fixing Settings + Preview + Download section styling...');
        
        // Find the Settings + Preview + Download section
        const allElements = document.querySelectorAll('*');
        let settingsSection = null;
        
        for (const element of allElements) {
            if (element.textContent && element.textContent.includes('Settings + Preview + Download')) {
                settingsSection = element.closest('section') || 
        ...
        if (settingsSection) {
            console.log('📦 Found Settings + Preview + Download section');
            
            // Apply consistent styling to match other containers
            ...
            // Remove all HR lines and faint separators
            const hrElements = settingsSection.querySelectorAll('hr');
            hrElements.forEach(hr => {
                console.log('🗑️ Removing HR line');
                hr.remove();
            });
            
            // Remove faint border lines between sections
            const allDivs = settingsSection.querySelectorAll('div');
            allDivs.forEach(div => {
                if (div.style.borderTop || div.style.borderBottom) {
                    div.style.borderTop = 'none';
                    div.style.borderBottom = 'none';
                    console.log('🗑️ Removed faint border line');
                }
            });
            
            // Add proper spacing between View Options and Download Options
            const viewOptionsText = settingsSection.querySelector('*:contains("View Options")');
            const downloadOptionsText = settingsSection.querySelector('*:contains("Download Options")');
            
            // Find View Options and Download Options sections
            let viewOptionsSection = null;
            let downloadOptionsSection = null;
            
            const allSectionDivs = settingsSection.querySelectorAll('div');
            for (const div of allSectionDivs) {
                if (div.textContent && div.textContent.trim() === 'View Options') {
                    viewOptionsSection = div.parentElement;
                }
                if (div.textContent && div.textContent.trim() === 'Download Options') {
                    downloadOptionsSection = div.parentElement;
                }
            }
            
            // Add spacing between View Options and Download Options
            if (viewOptionsSection && downloadOptionsSection) {
                viewOptionsSection.style.marginBottom = '20px';
                downloadOptionsSection.style.marginTop = '20px';
                console.log('✅ Added spacing between View Options and Download Options');
            }
            
            // Style the button rows consistently
            const buttonRows = settingsSection.querySelectorAll('div');
            ...
            console.log('✅ Settings + Preview + Download section styled consistently');
        } else {
            console.warn('⚠️ Settings + Preview + Download section not found');
        }
    }

    // 10. MOVE CONTAINERS 3,4,5 INSIDE WHITE FORM STRUCTURE - REAL DOM MANIPULATION
    function moveContainersInsideWhiteForm() {
        console.log('🎯 MOVING containers 3,4,5 INSIDE white form structure...');
        
        // Find the main white form container (where containers 1 & 2 are)
        const mainForm = document.querySelector('form') || document.querySelector('.container');
        if (!mainForm) {
            console.warn('⚠️ Main form container not found');
            return;
        }
        
        // Find the last container that's properly inside the form (Map Location)
        let insertionPoint = null;
        const formChildren = mainForm.children;
        ...
        if (!insertionPoint) {
            console.warn('⚠️ Could not find insertion point (Map Location container)');
            return;
        }
        
        // Find and move containers 3, 4, 5
        ...
        containersToMove.forEach(selector => {
            ...
            if (targetContainer) {
                console.log(`📦 Found ${selector.name}: ${selector.text} - MOVING INSIDE FORM`);
                
                // Style the container to match Map Location (shadowed, thick border)
                ...
                // ACTUALLY MOVE THE CONTAINER INTO THE FORM
                mainForm.insertBefore(targetContainer, insertionPoint.nextSibling);
                insertionPoint = targetContainer; // Update insertion point for next container
                
                console.log(`✅ ${selector.name} MOVED INSIDE white form structure`);
            } else {
                console.warn(`⚠️ Could not find ${selector.name}: ${selector.text}`);
            }
        });
        
        console.log('✅ All containers moved inside white form structure');
    }

    // 11. REMOVE ALL FAINT HR LINES GLOBALLY
    function removeAllFaintHRLines() {
        console.log('🎯 Removing ALL faint HR lines globally...');
        ...
        // Add CSS to prevent HR lines from appearing
        const style = document.createElement('style');
        style.textContent = `
            /* Remove all faint HR lines */
            hr {
                display: none !important;
            }
            
            /* Remove faint border lines */
        ...
        console.log('✅ All faint HR lines removed globally');
    }

    // 12. SAFE CONTAINER LAYOUT FIX - MOVE CONTAINERS 3,4,5 INSIDE WHITE FORM PAGE
    function fixContainerLayoutSafely() {
        console.log('🎯 SAFELY moving containers 3,4,5 inside white form page...');
        
        // Find the main form container (where containers 1 & 2 are properly positioned)
        const mainForm = document.querySelector('form') || document.querySelector('.container');
        if (!mainForm) {
            console.warn('⚠️ Main form container not found');
            return;
        ...
        // Find containers that are currently outside the form
        ...
        containerSelectors.forEach(selector => {
            ...
            for (const element of allElements) {
                if (element.textContent && element.textContent.includes(selector.text)) {
                    // Find the parent container (fieldset, section, or div with class)
                    targetContainer = element.closest('fieldset') || 
                                    element.closest('section') || 
                                    element.closest('.settings-row') ||
                                    element.closest('div[style*="background"]');
            ...
            if (targetContainer) {
                console.log(`📦 Found ${selector.name}: ${selector.text}`);
                
                // Check if it's already inside the main form
                const isInsideForm = mainForm.contains(targetContainer);
                
                if (!isInsideForm) {
                    console.log(`🔄 Moving ${selector.name} inside main form...`);
                    
                    // SAFE APPROACH: Use CSS positioning instead of DOM manipulation
                    // This preserves the existing layout while visually positioning it correctly
                    targetContainer.style.cssText += `
                    ...
                    // Ensure it has the same styling as containers 1 & 2
                    if (!targetContainer.style.backgroundColor || 
                        !targetContainer.style.backgroundColor.includes('#f8f9fa')) {
                        targetContainer.style.backgroundColor = '#f8f9fa';
                        targetContainer.style.border = '1px solid #e9ecef';
                        targetContainer.style.borderRadius = '5px';
                        targetContainer.style.padding = '15px';
                        targetContainer.style.marginBottom = '15px';
                    }
                    
                    console.log(`✅ ${selector.name} positioned safely`);
                } else {
                    console.log(`✅ ${selector.name} already inside form`);
                }
            } else {
                console.warn(`⚠️ Could not find ${selector.name}: ${selector.text}`);
            }
        ...
        // Add CSS to ensure consistent container appearance
        const style = document.createElement('style');
        style.textContent = `
            /* Ensure all containers have consistent styling */
            fieldset, .settings-row, section {
                background-color: #f8f9fa !important;
                border: 1px solid #e9ecef !important;
                border-radius: 5px !important;
                padding: 15px !important;
            ...
            /* Ensure input fields have white backgrounds */
            ...
            /* Ensure consistent container widths without breaking responsive design */
            .container, form {
                max-width: 100% !important;
                margin: 0 auto !important;
        ...
        console.log('✅ Container layout fixed safely with CSS positioning');
    }
    ...
    function initialize() {
        ...
        fixCombinedViewSpacingToMatchMockup(); // NEW: Fix spacing to match mockup
        removeAllFaintHRLines(); // NEW: Remove all faint HR lines globally
        moveContainersInsideWhiteForm(); // NEW: Move containers 3,4,5 inside white form structure
        fixSettingsPreviewDownloadStyling(); // NEW: Fix Settings + Preview + Download styling
        fixContainerLayoutSafely(); // NEW: Safe container layout fix
        forceAddZoomSlider();
        fixCanvasOrientations();
        fixCanvasHeight();
        setupTooltipUpdates(); // Add tooltip functionality
        
        console.log('✓ All fixes applied including container movement and HR line removal');
