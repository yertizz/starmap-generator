...
(function() {
    ...
    function syncFixedLayersValues() {
        ...
        // Add the blue "Fixed Layers" title if it doesn't exist
        function addFixedLayersTitle() {
            ...
            if (fixedDateValue) {
                ...
                if (dateInputGroup && !dateInputGroup.previousElementSibling?.classList?.contains('fixed-layers-title')) {
                    ...
                    console.log('✓ Fixed Layers title added before Date row');
                }
            }
        }
        ...
        function updateTextPlacementsDisplay() {
            // Update Text Placements date value
            const dateInput = document.getElementById('date');
            const timeInput = document.getElementById('time');
            const showTimeToggle = document.getElementById('show-time-toggle');
            
            // Find Text Placements date field (could have various IDs/classes)
            const textPlacementDateFields = document.querySelectorAll(
                '.text-placement-date-value, [id*="text-placement-date"], [class*="text-placement-date"]'
            );
            
            if (dateInput && textPlacementDateFields.length > 0) {
                let dateText = '';
                if (dateInput.value) {
                    // FIX: Parse date correctly to avoid "out by one" bug
                    const dateParts = dateInput.value.split('-');
                ...
                textPlacementDateFields.forEach(field => {
                    field.textContent = dateText;
                });
                console.log('✓ Text Placements date updated:', dateText);
            }
            
            // Update Text Placements coordinates value
            const latLongDisplay = document.getElementById('latLongDisplay');
            const textPlacementCoordsFields = document.querySelectorAll(
                '.text-placement-coords-value, [id*="text-placement-coords"], [class*="text-placement-coords"]'
            );
            
            if (latLongDisplay && textPlacementCoordsFields.length > 0) {
                const coordsText = latLongDisplay.textContent.trim();
                if (coordsText && coordsText !== 'Lat: ... | Long: ...') {
                    textPlacementCoordsFields.forEach(field => {
                        field.textContent = coordsText;
                    });
        ...
        // Add the title first
        setTimeout(addFixedLayersTitle, 500);
