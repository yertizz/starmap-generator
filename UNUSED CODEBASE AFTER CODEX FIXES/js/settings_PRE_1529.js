// START OF CODE - Cline - 2025-04-20 09:53 File: js/settings.js
...
function saveCurrentSettings() {
    ...
    try {
        ...
        const settings = {
            ...
            imageFormat: document.querySelector('input[name="image-format"]:checked')?.value || "png",
            pngTransparency: getChecked("png-transparency"),

            // Section 6: Star Map Image Settings
            starMapStyle: getVal("star-map-style", 'default'),
            circleRadiusPercent: getVal("circle-radius-percent", '90'), // Added
            borderWidth: getVal("border-width", '1'), // Added
            borderColor: getVal("border-color", '#FFFFFF'), // Added
            ...
            textPlacements: {
                entry1: {
                    ...
                    position: document.querySelector('input[name="text-placement-pos-1"]:checked')?.value || 'below'
                },
                entry2: {
                    order: getVal('text-placement-order-2', '2'),
                    position: document.querySelector('input[name="text-placement-pos-2"]:checked')?.value || 'below'
                },
                entry3: {
                    order: getVal('text-placement-order-3', '3'),
                    position: document.querySelector('input[name="text-placement-pos-3"]:checked')?.value || 'below'
                },
                date: {
                    order: getVal('text-placement-order-date', '4'),
                    position: document.querySelector('input[name="text-placement-pos-date"]:checked')?.value || 'below'
                },
                coords: {
                    order: getVal('text-placement-order-coords', '5'),
                    position: document.querySelector('input[name="text-placement-pos-coords"]:checked')?.value || 'below'
                }
            },

            timestamp: new Date().getTime()
        };
        // console.log("DEBUG: Basic settings gathered."); // Reduce noise
        // console.log("DEBUG: Advanced options object:", settings.advancedOptions); // Reduce noise

        // Save text layers
        // console.log("DEBUG: Gathering text layers..."); // Reduce noise
        for (let i = 1; i <= 3; i++) {
        ...
        // console.log("DEBUG: Text layers gathered."); // Reduce noise
        ...
        try {
            ...
            // console.log("Settings JSON:", settingsJSON); // Reduce noise
...
function loadSavedSettings() {
    ...
    try {
        ...
        // console.log("Retrieved from localStorage:", savedSettingsJSON); // Reduce noise
    } catch (storageError) {
    ...
    try {
        ...
        // console.log("Parsed settings object:", settings); // Reduce noise
    } catch (parseError) {
    ...
    try {
        ...
        if (occasionSelect && settings.occasion) {
             // If the saved occasion is a custom one (starts with custom_)
             // ensure it exists in the dropdown before selecting it.
             if (settings.occasion.startsWith('custom_') && settings.customOccasionText) {
                 // Check if this specific custom option already exists
                 let existingOption = occasionSelect.querySelector(`option[value="${settings.occasion}"]`);
                 if (!existingOption) {
                     // If not, add it (don't save again here, just add to UI for selection)
                     addCustomOccasionOption(settings.customOccasionText, settings.occasion, false);
                 }
                 // Now select it
                 occasionSelect.value = settings.occasion;
             } else if (settings.occasion !== 'custom') { // Handle standard or previously added custom options
                 occasionSelect.value = settings.occasion;
             }
             // Hide custom input by default on load
        ...
        setVal("circle-radius-percent", settings.circleRadiusPercent); // Added
        setVal("border-width", settings.borderWidth); // Added
        setColor("border-color", settings.borderColor); // Added
...
// END OF CODE - Cline - 2025-04-20 09:53 File: js/settings.js
