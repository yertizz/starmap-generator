/* START OF CODE - Cline - 2025-05-20 19:41:27 File: js/fixed-layers-fix-exact.js */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Force immediate update of date and coordinates
    forceInitialUpdate();
});
...
function addFixedLayersTitle() {
    try {
        ...
        const legendElement = document.querySelector('fieldset legend');
        if (!legendElement) {
    ...
    try {
        ...
        function updateFixedLayersCoordinates() {
            ...
            if (latInput && lonInput) {
                const coordsText = `${latInput.value || 'N29° 18.48263\''} | ${lonInput.value || 'W94° 47.83479\''}`;
...
function forceInitialUpdate() {
    try {
        console.log("Forcing initial update of date and coordinates");
        
        // Set default coordinates if none are present
        const latInput = document.getElementById('latitude');
        const lonInput = document.getElementById('longitude');
        
        if (latInput && !latInput.value) {
            latInput.value = 'N29° 18.48263\'';
        }
        
        if (lonInput && !lonInput.value) {
            lonInput.value = 'W94° 47.83479\'';
        }
        ...
        if (lonInput) {
            const event = new Event('input');
            lonInput.dispatchEvent(event);
        }
        
        // Set default date if none is present
        const dateInput = document.getElementById('date');
        if (dateInput && !dateInput.value) {
            dateInput.value = '2004-05-22';
        }
        
        // Set default time if none is present
        const timeInput = document.getElementById('time');
        if (timeInput && !timeInput.value) {
            timeInput.value = '12:01';
        }
        
        // Trigger change event to update date display
        if (dateInput) {
            const event = new Event('change');
            dateInput.dispatchEvent(event);
        }
        
        // Also directly update the Text Placements date
        const textPlacementsDate = document.getElementById('text-placement-content-date');
        if (textPlacementsDate && dateInput) {
            // Format the date
            ...
            if (!isNaN(date.getTime())) {
                ...
                formattedDate = 'May 22, 2004, 12:01 PM';
            }
            
            textPlacementsDate.textContent = formattedDate;
            console.log("Directly updated Text Placements date to:", formattedDate);
        }
        
        console.log("Successfully forced initial update of date and coordinates");
    } catch (error) {
        console.error("Error forcing initial update:", error);
    }
}

/* END OF CODE - Cline - 2025-05-20 19:41:27 File: js/fixed-layers-fix-exact.js */
