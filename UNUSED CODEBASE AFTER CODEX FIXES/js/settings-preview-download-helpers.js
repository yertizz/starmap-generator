 ...
 * ENHANCED VERSION - Properly clears Canvas Layout state
 */
function resetCanvasToUserDimensions() {
    ...
    // CRITICAL: Clear Canvas Layout state to prevent hijacking
    if (window.canvasLayoutState && window.canvasLayoutState.active) {
        console.log("🚫 Clearing Canvas Layout state to prevent hijacking");
        window.canvasLayoutState.active = false;
    }
    ...
    // Clear any background color set by Canvas Layout
    canvas.style.backgroundColor = '';
    
    // Remove any data attributes set by Canvas Layout
    if (canvas.dataset.bgColor) {
        delete canvas.dataset.bgColor;
    }
    
    console.log(`🔄 Reset canvas to user dimensions: ${dimensions.width}w x ${dimensions.height}h`);
    console.log("✅ Canvas Layout hijacking prevention applied");
