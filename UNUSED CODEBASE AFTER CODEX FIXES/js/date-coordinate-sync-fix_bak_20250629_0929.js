...
(function() {
    ...
    // CRITICAL FIX B: Add missing font-size dropdowns to Fixed Layers
    function addMissingFontSizeDropdowns() {
        ...
        function createFontSizeDropdown(id) {
            ...
            // Add font size options (same as customizable text layers)
            const fontSizes = ['8px', '10px', '12px', '14px', '16px', '18px', '20px', '22px', '24px', '26px', '28px', '30px', '32px', '36px', '40px', '48px', '56px', '64px', '72px'];
        ...
        // Find Fixed Layers section by looking for elements with "Date:" and "Lat/Long:" text
        const allElements = document.querySelectorAll('*');
        let dateRow = null;
        let coordsRow = null;
        
        for (const element of allElements) {
            if (element.textContent && element.textContent.trim().startsWith('Date:')) {
                dateRow = element.closest('div');
            }
            if (element.textContent && element.textContent.trim().startsWith('Lat/Long:')) {
        ...
        if (dateRow) {
            console.log('Adding font-size dropdown to Date row...');
        ...
        if (coordsRow) {
            console.log('Adding font-size dropdown to Lat/Long row...');
            ...
            if (fontFamilySelect && !coordsRow.querySelector('#fixed-coords-font-size')) {
                ...
                fontFamilySelect.parentNode.insertBefore(fontSizeDropdown, fontFamilySelect.nextSibling);
                console.log('✓ Font-size dropdown added to Lat/Long row');
            }
        }
    }
