...
async function setupAllHistories() { // MODIFIED: async (though awaits inside setupHistoryForInput are handled there)
    ...
    // ADDED: Setup for Event dropdown (hooks into existing modal)
    setupEventDropdownSuggestions('occasion', 'eventHistory', 'occasion-suggestions', 50);
    ...
    try {
        ...
        if (history.length > 0) {
            ...
            console.error("DEBUG HISTORY: Test failed! History system is not working correctly.");
            alert("History system test failed. Check console for details.");
        }
    } catch (error) {
        console.error("DEBUG HISTORY: Error testing history system:", error);
        alert("Error testing history system: " + error.message);
    }
}
...
/**
 * Sets up event dropdown suggestions that appear when clicking the select (like Address/Zip)
 * Hooks into the existing modal system for "Add Your Own..." functionality
 * @param {string} selectId - The ID of the select element ('occasion')
 * @param {string} storageKey - The key for storing history ('eventHistory')
 * @param {string} suggestionsContainerId - The ID of suggestions container
 * @param {number} maxItems - Maximum total items (50)
 */
function setupEventDropdownSuggestions(selectId, storageKey, suggestionsContainerId, maxItems = 50) {
    console.log(`DEBUG HISTORY: Setting up event dropdown suggestions for #${selectId}`);
    
    const selectElement = document.getElementById(selectId);
    const suggestionsContainer = document.getElementById(suggestionsContainerId);
    
    if (!selectElement) {
        console.error(`DEBUG HISTORY: Select element #${selectId} not found for event dropdown setup`);
        return;
    }
    
    if (!suggestionsContainer) {
        console.error(`DEBUG HISTORY: Suggestions container #${suggestionsContainerId} not found for event dropdown setup`);
        return;
    }

    // Protected events (not deletable) - built-in events (FIXED: using actual text values)
    const PROTECTED_EVENTS = [
        'Mother\'s Day', 'Father\'s Day', 'Birthday', 'Day We Met', 
        'Our Wedding Day', 'Day We Were Engaged', 'Sweet 16 Birthday',
        '21st Birthday', 'Graduation Day', 'Finally Retired'
    ];

    // Show suggestions when select is clicked (FIXED: using click instead of focus)
    selectElement.addEventListener('click', async function() {
        console.log(`DEBUG HISTORY: Event dropdown #${selectId} clicked, showing suggestions`);
        await displayEventSuggestions(selectId, storageKey, suggestionsContainerId, PROTECTED_EVENTS);
    });

    // Hide suggestions when focus leaves
    selectElement.addEventListener('blur', function() {
        console.log(`DEBUG HISTORY: Event dropdown #${selectId} blurred, will check if should hide suggestions`);
        setTimeout(() => {
            const focusedElement = document.activeElement;
            if (!suggestionsContainer.contains(focusedElement)) {
                console.log(`DEBUG HISTORY: Focus not inside suggestions container, hiding #${suggestionsContainerId}`);
                suggestionsContainer.style.display = 'none';
            } else {
                console.log(`DEBUG HISTORY: Focus still inside suggestions container, keeping #${suggestionsContainerId} visible`);
            }
        }, 300);
    });

    // Hook into existing modal system - listen for when custom occasions are added
    // This will be triggered by the existing handleOccasionChange function in main_app.js
    selectElement.addEventListener('change', async function() {
        console.log(`DEBUG HISTORY: Event dropdown #${selectId} changed to: "${this.value}"`);
        
        // If a custom option was selected (not the "Add Your Own..." trigger)
        if (this.value !== 'custom' && this.value !== '') {
            const selectedText = this.options[this.selectedIndex]?.text;
            if (selectedText && selectedText !== 'Add Your Own...') {
                console.log(`DEBUG HISTORY: Saving selected event to history: "${selectedText}"`);
                await addHistoryEntry(storageKey, selectedText, maxItems);
            }
        }
    });

    console.log(`DEBUG HISTORY: Event dropdown suggestions setup complete for #${selectId}`);
}

/**
 * Displays event suggestions with protected items logic (like Address/Zip dropdowns)
 * @param {string} selectId - The ID of the select element
 * @param {string} storageKey - The storage key for history
 * @param {string} suggestionsContainerId - The ID of suggestions container
 * @param {string[]} protectedEvents - Array of protected event values that can't be deleted
 */
async function displayEventSuggestions(selectId, storageKey, suggestionsContainerId, protectedEvents) {
    console.log(`DEBUG HISTORY: Displaying event suggestions for #${selectId}`);
    
    const suggestionsContainer = document.getElementById(suggestionsContainerId);
    const selectElement = document.getElementById(selectId);
    
    if (!suggestionsContainer || !selectElement) {
        console.error(`DEBUG HISTORY: Missing elements for event suggestions display`);
        return;
    }

    const userHistory = await getHistoryArray(storageKey);
    suggestionsContainer.innerHTML = '';

    if (userHistory.length > 0) {
        console.log(`DEBUG HISTORY: Building event suggestions dropdown with ${userHistory.length} items`);
        ...
        userHistory.forEach(item => {
            ...
            textSpan.addEventListener('click', function() {
                console.log(`DEBUG HISTORY: Clicked event suggestion "${item}"`);
                
                // Find or create option in select
                let option = Array.from(selectElement.options).find(opt => opt.textContent === item);
                if (!option) {
                    // Create new option and add it before "Add Your Own..."
                    option = document.createElement('option');
                    option.value = item.toLowerCase().replace(/\s+/g, '_');
                    option.textContent = item;
                    option.dataset.custom = 'true'; // Mark as user-added
                    
                    const addYourOwnOption = selectElement.querySelector('option[value="custom"]');
                    if (addYourOwnOption) {
                        selectElement.insertBefore(option, addYourOwnOption);
                    } else {
                        selectElement.appendChild(option);
                    }
                    console.log(`DEBUG HISTORY: Created new option for "${item}"`);
                }
                ...
                console.log(`DEBUG HISTORY: Selected event "${item}" and triggered change event`);
            });

            itemDiv.appendChild(textSpan);

            // Add delete button only for user-added items (not protected)
            const isProtected = protectedEvents.some(protected => 
                item === protected
            );
            ...
            if (!isProtected) {
                ...
                deleteBtn.addEventListener('click', async function(e) {
                    e.stopPropagation();
                    console.log(`DEBUG HISTORY: Deleting event suggestion "${item}"`);
                    await removeHistoryEntry(storageKey, suggestionsContainerId, item, selectId);
                    
                    // Also remove from dropdown if it exists and is custom
                    const optionToRemove = Array.from(selectElement.options).find(option =>
                        option.textContent === item && option.dataset.custom === 'true'
                    );
                    if (optionToRemove) {
                        selectElement.removeChild(optionToRemove);
                        console.log(`DEBUG HISTORY: Removed custom option from dropdown: "${item}"`);
                        
                        // Reset selection if this was selected
                        if (selectElement.value === optionToRemove.value) {
                            selectElement.value = '';
                            selectElement.dispatchEvent(new Event('change', { bubbles: true }));
                        }
                    }
                ...
                itemDiv.appendChild(deleteBtn);
                console.log(`DEBUG HISTORY: Added delete button for user event "${item}"`);
            } else {
                console.log(`DEBUG HISTORY: Protected event "${item}" - no delete button`);
            }
        ...
        console.log(`DEBUG HISTORY: Showing event suggestions container #${suggestionsContainerId}`);
    } else {
        suggestionsContainer.style.display = 'none';
        console.log(`DEBUG HISTORY: No event history items, hiding container #${suggestionsContainerId}`);
    }
}
