...
(function() {
    ...
    function moveContainersInsideForm() {
        ...
        // Try multiple ways to find the Map Location container
        for (const child of formChildren) {
            if (child.textContent && (
                child.textContent.includes('Map Location') ||
                child.textContent.includes('Latitude') ||
                child.textContent.includes('Longitude') ||
                child.querySelector('#latitude') ||
                child.querySelector('#longitude') ||
                child.querySelector('#zip-code')
            )) {
                insertionPoint = child;
                console.log('Found insertion point:', child);
                break;
            }
        }
        
        if (!insertionPoint) {
            // Fallback: use the last child in the form
            insertionPoint = formChildren[formChildren.length - 1];
            console.warn('Using fallback insertion point:', insertionPoint);
        }
    ...
    function syncFixedLayersValues() {
        ...
        // DISABLED: Periodic sync causing console spam
    ...
    function initialize() {
        ...
        moveContainersInsideForm(); // RE-ENABLED - MOVES CONTAINERS BACK INSIDE FORM
        fixCombinedViewOptionsUI();
        removeFaintHRLines();
        protectViewFunctions();
        syncFixedLayersValues();
        
        // DISABLED: Periodic checks causing infinite loop and console spam
        // setInterval(() => {
